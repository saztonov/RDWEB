-- =============================================================================
-- Единый скрипт развёртывания БД для OCR Web MVP
-- Дата: 2026-04-05
-- Описание: 18 таблиц, 6 enum-типов, RLS-политики, индексы, триггеры, seed-данные
--
-- ИНСТРУКЦИЯ:
--   1. Создайте новый проект на supabase.com
--   2. Откройте SQL Editor
--   3. Вставьте ВЕСЬ этот скрипт и выполните
--
-- ВАЖНО:
--   - Скрипт предназначен ТОЛЬКО для чистой БД (первичное развёртывание)
--   - Повторный запуск вызовет ошибки (таблицы уже существуют)
--   - FK на auth.users и функции auth.uid()/auth.jwt() — специфичны для Supabase
-- =============================================================================
...
CREATE TYPE parser_strategy AS ENUM ('plain_text', 'stamp_json', 'image_json', 'html_fragment');

-- ─────────────────────────────────────────────────────────────────────────────
-- Секция 3: Helper-функции
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ── Document Profiles + Documents + Pages ───────────────────────────────────
...
-- ── Prompt Templates + Profile Routes ───────────────────────────────────────
...
-- blocks: dirty detection + manual lock
CREATE INDEX idx_blocks_no_signature ON blocks(document_id)
    WHERE deleted_at IS NULL AND last_recognition_signature IS NULL;
CREATE INDEX idx_blocks_locked ON blocks(document_id)
...
-- ── Document Pages (через documents) ────────────────────────────────────────
...
-- ── OCR Sources — чтение всем, запись админам ───────────────────────────────
...
-- ── System Events — только admin/service ────────────────────────────────────
